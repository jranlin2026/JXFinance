...
const DEFAULT_SHIPPING_FEE = 2.4;
...
function parseShippingFee(value: unknown) {
  if (value === undefined || value === null || value === "") return DEFAULT_SHIPPING_FEE;
  const shippingFee = Number(value);
  if (!Number.isFinite(shippingFee) || shippingFee < 0) {
    throw new Error("单个包裹快递成本必须是大于或等于 0 的数字。");
  }
  return shippingFee;
}
...
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const shippingFee = parseShippingFee(req.body.shippingFee);
      const input = {
        store,
